  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post(
        ...
        { issue: issue.trim(), details: details.trim() },
        config
      );
  ...
  return (
    <div className="absolute inset-0 bg-app-drawer text-app-text-primary z-50 flex flex-col transition-transform duration-300 transform translate-x-0 select-none">
      ...
      <div className="h-[60px] bg-app-header flex items-center p-4 gap-4 border-b border-app-border shrink-0">
        ...
        <span className="text-app-text-primary font-semibold text-lg">Help & Feedback</span>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-6 flex flex-col">
        {success ? (
          <div className="flex-1 flex flex-col items-center justify-center text-center space-y-4">
            ...
            <h3 className="text-lg font-bold text-app-text-primary">Thank You!</h3>
            <p className="text-xs text-app-text-secondary max-w-[280px] leading-relaxed">
              Your feedback has been successfully submitted to the Vertex Connect team at{" "}
              <span className="text-brand font-semibold">vertexconnect.team@gmail.com</span>. We will review it shortly.
            </p>
            <button
              onClick={onClose}
              className="mt-6 px-6 py-2.5 bg-brand hover:bg-brand/90 text-white rounded-xl text-xs font-bold transition shadow-md"
            >
              Back to Settings
            </button>
          </div>
          ...
          <form onSubmit={handleSubmit} className="flex-1 flex flex-col justify-between">
            <div className="space-y-5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-app-text-primary font-semibold text-sm">Feedback & Support</h3>
            ...
            <button
              type="submit"
              disabled={submitting}
              className={`w-full py-3.5 rounded-xl text-white text-xs font-bold shadow-md transition flex items-center justify-center gap-2 ${
